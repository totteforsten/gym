...
export function LibraryClient({ exercises }: { exercises: Exercise[] }) {
  ...
  const [difficulty, setDifficulty] = useState("Alla");
  const [bodyPart, setBodyPart] = useState("Alla");
  ...
  const filtered = useMemo(() => {
    ...
    return exercises.filter((e) => {
      ...
      if (difficulty !== "Alla" && e.difficulty !== difficulty) return false;
      if (bodyPart !== "Alla" && e.bodyPart !== bodyPart) return false;
      if (query) {
        const hay = (
          e.name +
          " " +
  ...
  return (
    <main className="px-5 py-6 sm:px-8 sm:py-8">
      <PageHeader
        eyebrow="Övningsbank"
        title="Hitta din övning"
        subtitle="Varje övning har en demonstrationsvideo och tekniktips. Filtrera för att bygga det perfekta passet."
        action={
          <Link
            href="/library/new"
            className="btn-primary inline-flex items-center gap-2 rounded-xl px-4 py-2.5 text-sm font-semibold"
          >
            <Plus className="h-4 w-4" /> Ny övning
          </Link>
        }
      />
      ...
      <div className="mt-6 flex flex-col gap-3">
        <div className="flex gap-3">
          <div className="relative flex-1">
            ...
            <input
              ...
              placeholder="Sök övningar, muskler, taggar…"
        ...
        {showFilters && (
          <div className="card animate-fade-up flex flex-col gap-4 p-4">
            <FilterRow label="Kategori" options={CATEGORIES} value={category} onChange={setCategory} />
            <FilterRow label="Svårighet" options={DIFFICULTIES} value={difficulty} onChange={setDifficulty} />
            <FilterRow label="Kroppsdel" options={BODY_PARTS} value={bodyPart} onChange={setBodyPart} />
          </div>
        )}
      </div>
      ...
      <div className="mt-6 flex items-center justify-between">
        <p className="text-sm text-[var(--color-muted)]">
          ...
          {filtered.length === 1 ? "övning" : "övningar"}
        </p>
        {(activeFilters > 0 || q) && (
          <button
            onClick={reset}
      ...
      {filtered.length ? (
        ...
        <div className="mt-4">
          <EmptyState
            ...
            description="Prova att justera din sökning eller rensa filtren."
            action={
              <button
                onClick={reset}
